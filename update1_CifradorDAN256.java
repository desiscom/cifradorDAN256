...
//Optimizacion Sep2018 para procesar bloques mas rapido en mensajes grandes
//
...
ALGORITMO:
2017-2018 Daniel Solis (c) - email: jdansolis@gmail.com
...
en cada caso se deben incluir los datos del credor del algoritmo.
...
public class CifradoDAN256 {
    ...
    CifradoDAN256() {
     ...
     public static void main( String[] args ) {
         ...
         ArrayList<byte[]> bloquesDecifrados = dan256.decifrar( bloques.get(0), clave );
    ...
    private String segmentoXor( String segmento ) {
        ...
        for( int j = 0; j < claveBinXor.length(); j++ ) {
            if( claveBinXor.substring( j, j + 1 ).startsWith( segmento.substring( j, j+1 ) ) ) {
                xor.append( "0" ); // 1 xor 1 - 0 xor 0 = 0
            } else {
                xor.append( "1" ); // 1 xor 0 - 0 xor 1 = 1
            }
        }
        return xor.toString();
    }
    ...
    String rotacionBitsR( String stringBinario, int nBits ) {
        ...
        StringBuilder tmp = new StringBuilder();
        tmp.append( rotacion );
        tmp.append( bitsFaltantes );
        tmp.append( stringBinario.substring( 0, stringBinario.length() - nBits ) );
        return tmp.toString();
    }
    ...
    String rotacionBitsL( String stringBinario, int nBits ) {
        ...
        StringBuilder tmp = new StringBuilder();
        stringBinario = tmp.append( bitsFaltantes ).append( stringBinario ).toString();
        tmp = new StringBuilder();
        tmp.append( stringBinario.substring( nBits, stringBinario.length() ) );
        tmp.append( stringBinario.substring( 0, nBits ) );
        return tmp.toString();
    }
